...
if (!empty($temporadas_ids)) {
    $ids_placeholders = implode(',', array_fill(0, count($temporadas_ids), '?'));
    $sql_temporadas_names = "SELECT Temporada_id, Temporada_nombre 
                             FROM Temporada 
                             WHERE Temporada_id IN ($ids_placeholders)";
    $stmt_temporadas_names = $conn->prepare($sql_temporadas_names);


    $types = str_repeat('s', count($temporadas_ids)); // Tipo 's' para string
    $stmt_temporadas_names->bind_param($types, ...$temporadas_ids);
    $stmt_temporadas_names->execute();
    $result_temporadas_names = $stmt_temporadas_names->get_result();

    if (!$result_temporadas_names) {
        die("Error en la consulta: " . $conn->error);
    }
...
<body>
    ...
    <div class="form-container">
        <form action="Tabla_goleadores.php" method="post">
